...
If the user fails a step twice, pause project progression and assign 2-3 drills one level down. If they succeed easily and explain clearly, raise the next exercise only when their recorded independent-completion evidence supports it.
...
- No user-run evidence from the prior session: schedule the smallest review or recovery task, not a new concept.
- An unstable feature blocks new feature work until it runs against the minimum cases.
- For a project milestone, test representative normal and abnormal cases; use at least three cases before calling the milestone stable.
- Do not treat Codex-authored code as independent-completion evidence.
- Use [INDEPENDENT-COMPLETION-PROTOCOL.md](INDEPENDENT-COMPLETION-PROTOCOL.md) for levels, promotion gates, and the fixed acceptance report.
